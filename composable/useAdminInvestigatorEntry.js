...
export function useAdminInvestigatorEntry() {
  ...
  // ステータス変更を監視して派生値を更新
  watch(() => formData.value.status.dex.default, (v) => {
    syncDerivedValues()
  })
  watch(() => formData.value.status.pow.default, (v) => {
    syncDerivedValues()
  })
  watch(() => formData.value.status.edu.default, (v) => {
    syncDerivedValues()
  })
  // CON/SIZ の変更でHPを再計算
  watch(() => formData.value.status.con.default, (v) => {
    syncDerivedValues()
  })
  watch(() => formData.value.status.siz.default, (v) => {
    syncDerivedValues()
  })
  ...
  const submit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        feature: formData.value.feature ? formData.value.feature.split(',').map(s => s.trim()) : [],
  ...
  return { formData, errors, submit, reset, fetchFromUrl, editingId, setEditingId, revertToOriginal, setImageFile, setCropData }
}
