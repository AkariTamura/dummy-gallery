...
import { adminInvestigatorList, deleteInvestigator, updateInvestigatorCreatedAt } from '@/util/api.js'
    ...
    try {
      ...
      list.value = (data || []).map(inv => {
        if (typeof inv.status === 'string') {
          try { inv.status = JSON.parse(inv.status) } catch { inv.status = {} }
        }
        if (typeof inv.skill === 'string') {
          try { inv.skill = JSON.parse(inv.skill) } catch { inv.skill = {} }
        }
        if (!inv.created_at) {
