  ...
  <div class="container">
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <fieldset class="panel">
        ...
        <div class="form-row">
          <label>通称名 <span style="color:red;">*</span></label><br>
        ...
        <div class="form-row-inline">
          <div class="form-row">
            <label>年齢 <span style="color:red;">*</span></label><br>
          ...
          <div class="form-row">
            <label>身長</label><br>
            <BaseInput v-model.number="formData.height" placeholder="cm" type="number" class="input-small" />
          </div>
        </div>

        <div class="form-row-inline">
          <div class="form-row">
            <label>職業 <span style="color:red;">*</span></label><br>
        ...
        <div class="form-row">
          <label>特徴表</label><br>
          <BaseTextarea v-model="formData.feature" placeholder="無ければ空白" class="textarea-small" block />
        </div>

        <div class="form-row">
          <label>詳細</label><br>
          <BaseTextarea v-model="formData.detail" placeholder="背景など" class="textarea-large" block />
        </div>

        <div class="form-row">
          <label>URL</label><br>
          <div class="url-row">
            <BaseInput v-model="formData.url" placeholder="キャラクター保管所のURL" class="input-full" block />
            <BaseButton type="button" variant="secondary" size="sm" @click="fetchFromUrl">URLから取得</BaseButton>
          </div>
        </div>
      </fieldset>
          ...
          <tbody>
            <tr>
              ...
              <td v-for="k in row1Keys" :key="'base-'+k" class="cell-base">
                <template v-if="selectRanges[k]">
                  <BaseSelect v-model="formData.status[k].default" :disabled="isDisabledStatus(k)" :class="{ isDisable: isDisabledStatus(k) }" class="status-select">
            ...
            <tr>
              ...
              <td v-for="k in row1Keys" :key="'ex1-'+k" class="cell-base">
                <BaseInput v-model.number="formData.status[k].ex1" type="number" class="status-input-small" />
              </td>
            </tr>
            ...
            <tr>
              ...
              <td v-for="k in row1Keys" :key="'ex2-'+k" class="cell-base">
                <BaseInput v-model.number="formData.status[k].ex2" type="number" class="status-input-small" />
              </td>
            </tr>
        ...
        <div class="lower-wrapper">
              ...
              <tbody>
                <tr>
                  ...
                  <td v-for="k in placedKeys" :key="'base2-'+k" class="cell-base">
                      <BaseInput v-model.number="formData.status[k].default" :disabled="isDisabledStatus(k)" :class="{ isDisable: isDisabledStatus(k) }" type="number" class="status-input-small" />
                  </td>
                </tr>
          ...
          <div class="right-box">
            <div class="right-block">
              ...
              <div class="ratio-row" :class="{ overcap: isJobOverCap }">{{ totalEx1 }}/{{ eduTotalCap }}</div>
            </div>

            <div class="right-block">
              <div class="section-title" :class="{ overcap: isInterestOverCap }">興味P</div>
              <div class="ratio-row" :class="{ overcap: isInterestOverCap }">{{ totalEx2 }}/{{ intTotalCap }}</div>
            </div>
          </div>
          <div class="right-box">
            <div class="right-block">
              ...
              <div class="san-row">
                <BaseInput v-model.number="formData.status.san.default" type="number" class="status-input-small" :class="{ overcap: isSanOverMax }" />
        ...
        <details v-for="categoryKey in skillCategories" :key="categoryKey" class="skill-details">
          ...
          <div class="skill-body" :class="{ overcap: isJobOverCap || isInterestOverCap }">
            <table class="skill-table">
              ...
              <tbody>
                <template v-for="(skillItem, skillKey) in formData.skill[categoryKey]" :key="skillKey">
                  <template v-if="typeof skillItem === 'object' && !Array.isArray(skillItem) && skillItem.label !== undefined">
                    <tr>
                      ...
                      <td class="skill-cell"><BaseInput v-model.number="skillItem.default" type="number" class="skill-input isDisable" disabled /></td>
                      <td class="skill-cell"><BaseInput v-model.number="skillItem.ex1" type="number" class="skill-input" /></td>
                      <td class="skill-cell"><BaseInput v-model.number="skillItem.ex2" type="number" class="skill-input" /></td>
                      <td class="skill-cell"><BaseInput v-model.number="skillItem.ex3" type="number" class="skill-input" /></td>
                      <td class="skill-cell"><BaseInput v-model.number="skillItem.ex4" type="number" class="skill-input" /></td>
                  ...
                  <template v-else-if="Array.isArray(skillItem)">
                    <tr v-for="(ex, exIdx) in skillItem" :key="`${skillKey}-${exIdx}`">
                      ...
                      <td class="skill-cell"><BaseInput v-model.number="ex.default" type="number" class="skill-input" /></td>
                      <td class="skill-cell"><BaseInput v-model.number="ex.ex1" type="number" class="skill-input" /></td>
                      <td class="skill-cell"><BaseInput v-model.number="ex.ex2" type="number" class="skill-input" /></td>
                      <td class="skill-cell"><BaseInput v-model.number="ex.ex3" type="number" class="skill-input" /></td>
                      <td class="skill-cell"><BaseInput v-model.number="ex.ex4" type="number" class="skill-input" /></td>
                      <td class="skill-cell total">{{ getSkillTotal(ex) }}</td>
                      <td class="skill-cell action">
                        <BaseButton type="button" variant="danger" size="sm" @click="removeExtraSkill(categoryKey, skillKey, exIdx)">削除</BaseButton>
                      </td>
                    </tr>
                  </template>
                </template>
            ...
            <div class="skill-extra">
              <template v-if="addingOpen[categoryKey]">
                <div class="skill-extra-row">
                  <BaseInput v-model="newExtra[categoryKey].name" placeholder="ラベル" class="skill-name-input" />
                  <BaseInput v-model.number="newExtra[categoryKey].default" type="number" placeholder="初期値" class="skill-input" />
                  <BaseInput v-model.number="newExtra[categoryKey].ex1" type="number" placeholder="職業P" class="skill-input" />
                  <BaseInput v-model.number="newExtra[categoryKey].ex2" type="number" placeholder="興味P" class="skill-input" />
                  <BaseInput v-model.number="newExtra[categoryKey].ex3" type="number" placeholder="成長P" class="skill-input" />
                  <BaseInput v-model.number="newExtra[categoryKey].ex4" type="number" placeholder="その他" class="skill-input" />
                  <BaseButton type="button" variant="primary" size="sm" @click="confirmAddExtra(categoryKey)">追加</BaseButton>
                  <BaseButton type="button" variant="secondary" size="sm" @click="cancelAddExtra(categoryKey)">キャンセル</BaseButton>
                </div>
              </template>
              <template v-else>
                <BaseButton type="button" variant="primary" size="sm" @click="openAddExtra(categoryKey)">＋ 追加</BaseButton>
              </template>
            </div>
          </div>
        </details>
...
const { formData, errors, submit, reset, fetchFromUrl, setEditingId, revertToOriginal, setImageFile, setCropData } = useAdminInvestigatorEntry()
...
const damageBonus = computed(() => {
  ...
  const str = (Number(strStatus.default) || 0) + (Number(strStatus.ex1) || 0) + (Number(strStatus.ex2) || 0)
  const siz = (Number(sizStatus.default) || 0) + (Number(sizStatus.ex1) || 0) + (Number(sizStatus.ex2) || 0)
  const total = str + siz
...
// 技能値の合計を取得
const getSkillTotal = (skillItem) => {
  const base = Number(skillItem?.default) || 0
  const ex1 = Number(skillItem?.ex1) || 0
  const ex2 = Number(skillItem?.ex2) || 0
  const ex3 = Number(skillItem?.ex3) || 0
  const ex4 = Number(skillItem?.ex4) || 0
...
const sumSkillPoints = (field) => {
  ...
  for (const category of Object.keys(skill)) {
    ...
    for (const key of Object.keys(items)) {
      const val = items[key]
      if (Array.isArray(val)) {
        val.forEach((ex) => { total += Number(ex?.[field]) || 0 })
...
const isJobOverCap = computed(() => (Number(totalEx1.value) || 0) > (Number(eduTotalCap.value) || 0))
const isInterestOverCap = computed(() => (Number(totalEx2.value) || 0) > (Number(intTotalCap.value) || 0))
...
const isSanOverMax = computed(() => (Number(formData.value.status.san?.default) || 0) > (Number(sanMax.value) || 0))
...
const placementIndexToKey = headerKeys.map((_, i) => Object.keys(lowerPlacement).find(k => lowerPlacement[k] === i) || null)
...
.form-row-inline { display:flex; gap:16px; flex-wrap:wrap; align-items:flex-end; margin-bottom:8px; }
.form-row-inline .form-row { margin-bottom:0; }
.url-row { display:flex; gap:8px; align-items:flex-end; }
.url-row .input-full { flex:1; }

/* Inputs */
.input-full { width:100%; padding:4px; box-sizing:border-box; }
.input-small { width:100px; padding:4px; box-sizing:border-box; }
.textarea-small { width:100%; height:80px; padding:4px; box-sizing:border-box; }
.textarea-large { width:100%; height:100px; padding:4px; box-sizing:border-box; }
.select-small { width:72px; padding:4px; text-align:center; box-sizing:border-box; }

/* Status tables */
.status-table { width:100%; border-collapse:collapse; text-align:center; }
.col-label { width:160px; padding:8px; border-bottom:1px solid #ddd; }
.col-head { padding:8px; border-bottom:1px solid #ddd; }
.col-head-title { font-weight:600; }
.row-label { text-align:left; padding:8px; font-weight:600; }
.cell-base { padding:8px; border-bottom:1px solid #f0f0f0; text-align:center; }
.status-select { width:72px; padding:4px; text-align:center; }
.status-input-small { width:72px; padding:4px; text-align:center; box-sizing:border-box; }
.row-label.current, .cell-current { background:#f7f7f7; }
.current-value { font-weight:600; padding:8px; }

/* Lower section */
.lower-wrapper { display:flex; width:100%; justify-content:start; gap:16px; margin-top:8px; }
.lower-left { width:80%; }
.lower-table { width:80%; border-collapse:collapse; text-align:center; }
.left.col-label { text-align:left; }

/* Right info box */
.right-box { width:20%; border:1px solid #eee; padding:30px; margin:10px; border-radius:4px; box-sizing:border-box; }
.right-block { margin-bottom:12px; }
.section-title { font-weight:600; margin-bottom:6px; }
.san-row { display:flex; align-items:center; justify-content:flex-end; gap:8px; }
.db-row { font-size:1.1em; display:flex; align-items:center; justify-content:flex-end; }
.ratio-row { font-size:1.05em; display:flex; align-items:center; justify-content:flex-end; }
.section-title.overcap, .ratio-row.overcap { color:#d00; }
.san-row .overcap { color:#d00; }

/* Skill area */
.skill-details { margin-bottom:16px; }
.skill-summary { cursor:pointer; font-weight:600; padding:8px; background:#f5f5f5; }
.edit-input { width:100%; padding:4px; border:1px solid #007bff; box-sizing:border-box; }
.edit-icon { cursor:pointer; margin-left:6px; color:#888; font-size:1.2em; }
.skill-input { width:100px; padding:4px; box-sizing:border-box; }
.skill-body { padding:12px; }
.skill-body.overcap { background:#ffdddd; }
.skill-table { width:100%; border-collapse:collapse; text-align:center; }
.skill-head { padding:6px; border-bottom:1px solid #ddd; font-weight:600; background:#f7f7f7; }
.skill-name { text-align:left; width:220px; }
.skill-cell { padding:6px; border-bottom:1px solid #f0f0f0; }
.skill-name-wrap { display:flex; align-items:center; gap:6px; }
.skill-name-text { user-select:none; }
.skill-cell.total { font-weight:600; }
...
.skill-cell.action { width:90px; }
.skill-extra { margin-top:8px; }
.skill-extra-row { display:flex; gap:8px; align-items:center; flex-wrap:wrap; }
.skill-name-input { flex:1; min-width:160px; max-width:250px; padding:4px; box-sizing:border-box; }
...
@media (max-width: 768px) {
  ...
  .lower-wrapper { flex-direction:column; }
  .lower-left { width:100%; }
  .right-box { width:100%; margin:0 0 12px; padding:16px; }
  .skill-name { width:160px; }
  .skill-extra-row { flex-direction:column; align-items:stretch; }
  .skill-name-input { max-width:none; }
}
</style>
