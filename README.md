...
## 依存API/DB（PHP/SQLite）の起動手順
1. `api/` と `db/` が配置されているディレクトリをPHPの公開ディレクトリとして設定
2. PHP の組み込みサーバーを起動
```bash
php -S localhost:8000
```
3. フロント側のAPI URL設定を、必要に応じて `util/api.js` で更新
